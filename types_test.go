...
func FixmeTestPackTransaction(t *testing.T) {
	...
	tx := &Transaction{
		...
		Actions: []*Action{
			{
				Account: AccountName("zswhq"),
				Name:    ActionName("transfer"),
				Authorization: []PermissionLevel{
					{AccountName("zswhq"), PermissionName("active")},
				},
			},
		},
	}
...
func TestAuthorityBinaryMarshal(t *testing.T) {
	key, err := ecc.NewPublicKey(ecc.PublicKeyPrefixK1Output + "6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV")
	require.NoError(t, err)
	a := Authority{
		Threshold: 2,
		Keys: []KeyWeight{
...
func TestActionNoData(t *testing.T) {
	...
	assert.Equal(t,
		`{"account":"zswhq","name":"transfer","authorization":[]}`,
		string(cnt),
	)
